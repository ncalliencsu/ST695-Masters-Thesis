    ...
    mainPanel(
      tabsetPanel(
        ...
        tabPanel("F-Test Results",
          ...
          downloadButton("download_F-Test", "Download F-Test Results")
        )
      )
    )
  )
...
server <- function(input, output, session) {
  ...
  observeEvent(input$submit, {
    ...
    sim_model_S <- function(X_S, mu_S, mu_W, Sigma2_S, Sigma2_W, WP) {
      ...
      rownames(rmodel_S_summary$coefficients) <- sub("\\(Intercept\\)", "Intercept", rownames(rmodel_S_summary$coefficients))
    ...
    sim_model_SW <- function(X_SW, Y_S, Y_W, X_S) {
      ...
      rownames(rmodel_SW1_summary$coefficients) <- sub("\\(Intercept\\)", "Intercept", rownames(rmodel_SW1_summary$coefficients))
      ...
      rownames(rmodel_SW2_summary$coefficients) <- sub("\\(Intercept\\)", "Intercept", rownames(rmodel_SW2_summary$coefficients))
    ...
    output$download_Ftest <- downloadHandler(
      filename = function() {
