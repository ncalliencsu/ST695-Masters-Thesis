...
server <- function(input, output, session) {
  ...
  output$x_inputs <- renderUI({
    lapply(1:input$HTC, function(i) {
      numericInput(paste0("x", i), paste0("x", i, " (real number):"), value = 0)
    })
  })
  # Dynamically generate numeric inputs for z parameters
  output$z_inputs <- renderUI({
    lapply(1:input$ETC, function(i) {
      numericInput(paste0("z", i), paste0("z", i, " (real number):"), value = 0)
    })
  })
  # Dynamically generate numeric inputs for interaction terms
  output$interaction_inputs <- renderUI({
    x_names <- paste0("x", 1:input$HTC)
    z_names <- paste0("z", 1:input$ETC)
    interaction_names <- c(
      if (length(x_names) > 1) combn(x_names, 2, FUN = function(x) paste(x, collapse = ":")),
      if (length(z_names) > 1) combn(z_names, 2, FUN = function(x) paste(x, collapse = ":")),
      as.vector(outer(x_names, z_names, paste, sep = ":"))
    )
  ...
  observeEvent(input$submit, {
    ...
    effects <- c(
      ...
      sapply(x_names, function(n) isolate(input[[n]])),
      sapply(z_names, function(n) isolate(input[[n]])),
      sapply(interaction_ids, function(n) isolate(input[[n]]))
    )
    # Create all combinations of x and z factors
    combined_list <- c(
      setNames(replicate(ETC, c(-1, 1), simplify = FALSE), z_names),
      setNames(replicate(HTC, c(-1, 1), simplify = FALSE), x_names)
    )
    ...
    for (n in x_names) effect_input_map[[n]] <- isolate(input[[n]])
    for (m in z_names) effect_input_map[[m]] <- isolate(input[[m]])
    ...
    output$download_params <- downloadHandler(
      ...
      content = function(file) {
        param_lines <- c(
          ...
          sapply(x_names, function(n) paste(n, "=", input[[n]])),
          sapply(z_names, function(n) paste(n, "=", input[[n]])),
