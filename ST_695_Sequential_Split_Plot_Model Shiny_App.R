...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      numericInput("sigma2_W", "sigma2_W (positive real):", value = 0.2, min = 0, step = 0.01),
      numericInput("sigma2_S", "sigma2_S (positive real):", value = 1, min = 0, step = 0.01),
  ...
  observeEvent(input$submit, {
    ...
    plot_overlaid_histograms <- function(S_Model, W_Model, SW_Model, bins = 20) {
      ...
      param_map <- c(
        ...
        setNames(sapply(w_names, function(n) input[[n]]), w_names),
        setNames(sapply(s_names, function(n) input[[n]]), s_names),
        setNames(sapply(interaction_names, function(n) input[[n]]), interaction_names),
        Sigma2_W = input$sigma2_W,
        Sigma2_S = input$sigma2_S
      )
      for (col in colnames(S_Model)) {
        ...
        label_text <- if (!is.null(param_value)) paste0(col, " = ", param_value) else ""
