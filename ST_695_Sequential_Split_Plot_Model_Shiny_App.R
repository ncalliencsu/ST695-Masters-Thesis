...
server <- function(input, output, session) {
  ...
  observeEvent(input$submit, {
    ...
    # --- REMOVE: Lenth Plot Output (UI and Server) ---
    # --- REMOVE: Lenth plot and download button to UI ---
    # --- REMOVE: Lenth plot renderers and download handlers ---
    # --- REMOVE: run_lenth function and all related variables ---
    ...
    output$tpr_table <- renderDataTable({
      ...
      pval_cols_SW <- grep("^pvalue_Model_SW", colnames(sim_df_SW), value = TRUE)
      ...
      tpr_SW <- colMeans(sim_df_SW[, pval_cols_SW, drop = FALSE] <= 0.05, na.rm = TRUE)
