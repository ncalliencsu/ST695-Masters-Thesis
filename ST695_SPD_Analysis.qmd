...
**Method 4B ANOVA using LMER and AFEX**
```{r S Model by lmer with WP 3-way interaction}
formula_4B <- as.formula(paste("Y_S ~ (", 
    paste(linear_terms, collapse = " + "),")^2 + T:S:P + (1|WP)"))
Method_4B_mixed <- lmer(formula_4B, data = Ys_data)
Method_4B_mixed
Method_4B_ANOVA <- anova(Method_4B_mixed, ddf = "Kenward-Roger")
Method_4B_ANOVA
```
...
formula_6S <- as.formula(paste("Y_S ~ (", 
    paste(linear_terms, collapse = " + "),")^2"))
model_6S <- lm(formula_6S, data = Ys_data)  # Use lm()
...
**Method 8 Sequential Split Plot Model**
```{r W Model and S-W Model}

#S Model - SP Factors (main SP effects, 2-way interactions
# and 2-way interactions with WP Factors)
formula_S <- as.formula(paste("Y_S ~ (", 
    paste(linear_terms, collapse = " + "),")^2"))
...
cat("Linear Model Fit for W Model", "\n")
W_Model <- summary(lm(formula_W, data = Yw_data))
W_Model
cat("\n")
cat("Linear Model Fit for SW Model", "\n")
SW_Model <- summary(lm(formula_SW, data = df_SW))
SW_Model
```
