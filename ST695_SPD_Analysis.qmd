...
cat(" Complete PLA Data Set\n")
print(Ys_data)

cat(" Whole Plot PLA Data Set\n")
print(Yw_data)
...
**Method 2 (Restricted Maximum Likelihood)**
```{r S Model by REML}
WP_factors <- colnames(Ys_data[1:3])
SP_factors <- colnames(Ys_data[4:5])
...
formula_wp <- as.formula(
  paste(
    "Denier ~ (", 
    paste(WP_factors, collapse = " + "), 
    ")^3"
  )
)
...
halfnormal(
  effects_wp,
  main = "Half-Normal Plot for S Model:\nWP Factors + 2 & 3-way Interactions",
  labs = effect_names_wp,
  alpha = 1
)


# Model for SP factors and WP:SP interactions
SP_terms <- paste(
  "(", 
  paste(SP_factors, collapse = " + "), 
  ")^2"
)

# Interactions between each WP_factor and each SP_factor
WP_SP_interactions <- paste(
  outer(
    WP_factors,
    SP_factors,
    function(w, s) paste(w, s, sep = ":")
  ),
  collapse = " + "
)
...
LenthPlot(effects, main = "Lenth Plot for W Model: All Effects")

```

**Method 8 Sequential Split Plot Method for W Model**
```{r W Model by SSP Method}

WP_factors <- colnames(Yw_data[1:3])


# Model for WP factors (main effects, 2-way interactions)
formula_wp <- as.formula(
  paste(
    "Denier ~ (",
    paste(WP_factors, collapse = " + "),
