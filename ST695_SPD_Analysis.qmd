...
Method_2A
```
**Method 2B (Restricted Maximum Likelihood using Kenward Roger with 2-Way Interactions)**
```{r}
# Model with all main effects, 2-way interactions, and random WP effect
...
**Method 2C (Restricted Maximum Likelihood using Kenward Roger with 3-Way Interactions)**
```{r}
# Model with all main effects, 3-way interactions without T:P:S, and random WP effect
formula_2C <- Y_S ~ (T + P + S + D + R)^3 - T:P:S + (1|WP)
# Fit model with Kenward-Roger method
model_2C <- lmer(formula_2C, data = Ys_data, REML = TRUE)
Method_2C <- summary(model_2C, ddf = "Kenward-Roger")
Method_2C
...
#-----------------------------------------------------------
#S Model - SP Factors (WP main effects, SP main effects, 
#and all 2-way and 3-way interactions (10 three-way interactions)
formula_S <- as.formula(paste("Y_S ~ (", paste(linear_terms, 
                                  collapse = " + "),")^3"))
#Create model matrix for S Model
Y_S <- Ys_data$Y_S
...
formula_SW <- as.formula(
  "Y_SW ~ D + R +
   `D:R` + `T:D` + `P:D` + `S:D` + `T:R` + `P:R` + `S:R` +
   `T:D:R` + `P:D:R` + `S:D:R` + `T:P:D` + `T:S:D` + `P:S:D` + `T:P:R` + `T:S:R` + `P:S:R`")
